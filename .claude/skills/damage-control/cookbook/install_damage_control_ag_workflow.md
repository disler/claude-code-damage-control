...
12. Copy all hook files to target:
```bash
cp -r [SOURCE_DIR]/* [TARGET_HOOKS_DIR]/
```

### Step 5: Install Settings Configuration

13. Read the appropriate settings template:
...
14. **For Fresh Install or Overwrite**:
...
16. **Cleanup**: Remove the settings template from the hooks directory (it was copied with the hooks but belongs in the parent):
```bash
rm [TARGET_HOOKS_DIR]/python-settings.json [TARGET_HOOKS_DIR]/typescript-settings.json 2>/dev/null || true
```
